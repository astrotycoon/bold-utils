...
function copy_template_header
{
  local NAME=$1
  local DEPLOYMENT=$2
  local TARGET_FILE="${BOLDTOP}/tools/bold/unittests/${NAME}Test.h";
...
function copy_template_impl
{
  local NAME=$1
  local DEPLOYMENT=$2
  local TARGET_FILE="${BOLDTOP}/tools/bold/unittests/${NAME}Test.cpp";
...
function replace_class
{
  ...
  local complete_path=`find ${BOLDTOP} | grep ${CLASS_NAME}\.h`
  DEPLOYMENT=`echo ${complete_path/*\/include\/bold\//} | cut -f 1 -d \/`
