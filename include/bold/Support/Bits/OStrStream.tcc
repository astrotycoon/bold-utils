...
//
//===----------------------------------------------------------------------===//
#ifndef BOLD_SUPPORT_OUTPUT_STRING_STREAM_TCC
#define BOLD_SUPPORT_OUTPUT_STRING_STREAM_TCC
...
BasicOStringBuf<CharT, Traits, Alloc>::length() const
{
                                              ...
                                              std::streamsize pN)
{
  ...
  while (result < pN) {
    const std::streamsize buf_len = length();
    if (buf_len) {
