...
// Diagnostic
//===----------------------------------------------------------------------===//
diagnostic::Diagnostic::Diagnostic(const Engine::State& pState,
                                   const InfoMap& pInfoMap)
...
{
  StringRef desc = m_InfoMap.description(id());
                                    ...
                                    std::string& pOutStr) const
{
  ...
  while (cur_char != pEnd) {
    ...
    Engine::ArgumentKind kind = getArgKind(arg_no);
    switch (kind) {
      case Engine::ak_std_string: {
        const std::string& str = getArgStdStr(arg_no);
        pOutStr.append(str.begin(), str.end());
        break;
      }
      case Engine::ak_c_string: {
...
{
  assert(Engine::ak_std_string == getArgKind(pID) &&
...
{
  assert(Engine::ak_c_string == getArgKind(pID) &&
         "Invalid argument accessor!");
...
{
  assert(Engine::ak_sint32 == getArgKind(pID) &&
         "Invalid argument accessor!");
...
{
  assert(Engine::ak_uint32 == getArgKind(pID) &&
         "Invalid argument accessor!");
...
{
  assert(Engine::ak_sint64 == getArgKind(pID) &&
         "Invalid argument accessor!");
...
{
  assert(Engine::ak_uint64 == getArgKind(pID) &&
         "Invalid argument accessor!");
...
{
  assert(Engine::ak_bool == getArgKind(pID) &&
         "Invalid argument accessor!");
